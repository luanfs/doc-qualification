...
\begin{thrm}
	...
	\begin{equation}
		...
		\int_{y_{j-\frac{1}{2}}}^{y_{j+\frac{1}{2}}}{f(x,y)\,dx \,dy}-f(x_i,y_j) = C_1 \Delta x^2 + C_2 \Delta y^2, 
	\end{equation}
	where $C_1$ and $C_2$ are constants that depend only on $f$, and $x_i = \frac{x_{i+\frac{1}{2}} + x_{i-\frac{1}{2}}}{2}$,
...
\begin{corollary}
	...
	as the union of $[x_{i-\frac{1}{2}},x_{i+\frac{1}{2}}] \times  [y_{j-\frac{1}{2}},y_{j+\frac{1}{2}}]$,
	$i,j=1, \ldots, N$,
	with lengths $\Delta x= \Delta y$, we have
	\begin{equation}
	\frac{1}{\Delta x \Delta y}\int_{a}^{b}\int_{c}^{d}
	{f(x,y)\,dx \,dy}-\sum_{i,j=1}^Nf(x_i,y_j) = C_1 \Delta x^2, 
\end{equation}
where $C_1$ and $C_2$ are constants that depend only on $f$.
\end{corollary}
\begin{proof}
...
\int_{x_{i-\frac{1}{2}}}^{x_{i+\frac{1}{2}}}
\int_{y_{j-\frac{1}{2}}}^{y_{j+\frac{1}{2}}} {f(x,y)\,dx \,dy}\\
 &=  \frac{1}{\Delta x \Delta y}\bigg[
\sum_{i,j=1}^N {f(x_i,y_j)} +
\sum_{i,j=1}^N \bigg({\partial^2_x f(\nu_i,y_j)}\frac{\Delta x^2}{24}+{\partial^2_y f(\nu_i,y_j)\frac{\Delta y^2}{24}}\bigg)\bigg].
\end{align*}
We notice that
\begin{align*}
\frac{1}{\Delta x \Delta y}
...
Combining this with corollary {anexo-numint-col2}, we have the second-order extrapolation formula as follows.
\begin{corollary}
