...
Let us assume that we are given $Q^n$.
We can fill its ghost cell values using the scheme \textbf{ET-ZA22} described in Section \ref{cs-interp}.
...
as in Section \ref{cs-recon}. Then, we may express the fluxes as:
\begin{align}
	\label{chp5-flux-xdir}
	F_{i+\frac{1}{2},j,p}^n ({Q^n_{\times,j,p},\tilde{u}^n_{i+\frac{1}{2},j,p}}) &= \tilde{u}^{n}_{i+\frac{1}{2},j,p}\\ 
	&\times
	\begin{cases}
...
\begin{align}
	...
	G_{i,j+\frac{1}{2},p}^n ({Q^n_{i,\times,p},\tilde{v}^n_{i,j+\frac{1}{2},p}}) &= \tilde{v}^{n}_{i,j+\frac{1}{2},p}\\
	&\times
	\begin{cases}
...
With these flux values, the discrete divergence (Equation \eqref{eqdiv-split}) can be computed for the interior cells,
and the solution can be updated for the next time step.
The wind needed in ghost positions is computed as in Section \ref{cs-wind-interp}.
The scheme described here requires modifying the flux at the edges to ensure mass conservation. 
We shall apply the schemes described in Section \ref{sec-csdsplit}.
When using the averaging flux scheme, we refer to the scheme as \textbf{ET-ZA22-AF}.
When using the projection scheme, we refer to the scheme as \textbf{ET-ZA22-PR}.

\subsubsection{ET-PL07}
Assuming that we have $Q^n$, let's consider the computation of all PPM stencils used in the \textbf{ET-PL07} method, as described in Section \ref{cs-interp}.
...
\begin{align}
	...
	\end{cases}
\end{align}
for $i=1, \ldots, N$, $j=1, \ldots, M$, $p=1,\ldots,6$ and we also construct a piecewise-parabolic
...
\begin{align}
	...
	\end{cases}
\end{align}
for $i=1, \ldots, N$, $j=1, \ldots, M$, $p=1,\ldots,6$. Then, we integrate the parabolas and 
\begin{align}
	\label{chp5-flux-xdir--pl07}
	F_{i+\frac{1}{2},j,p}^n ({Q^n_{\times,j,p},\tilde{u}^n_{i+\frac{1}{2},j,p}}) &={\sigma}_{i,j+\frac{1}{2},p} \tilde{u}^{n}_{i+\frac{1}{2},j,p}\\
	&\times
	\begin{cases}
	...
	\end{cases}
\end{align}
for $i=0, \ldots, N$, $j=1, \ldots, M$, $p=1,\ldots,6$ and 
\begin{align}
	\label{chp5-flux-ydir-pl07}
	G_{i,j+\frac{1}{2},p}^n ({Q^n_{i,\times,p},\tilde{v}^n_{i,j+\frac{1}{2},p}})&={\sigma}_{i,j+\frac{1}{2},p} \tilde{v}^{n}_{i,j+\frac{1}{2},p}\\
	&\times
	\begin{cases}
	...
	\end{cases}
\end{align}
for $i=1, \ldots, N$, $j=0, \ldots, M$, $p=1,\ldots,6$.
To compute the inner operators ($\mathbf{f}$ and $\mathbf{g}$) from the discrete divergence
(Equation \eqref{eqdiv-split}) for the interior cells, we can utilize the flux values obtained.
However, for the outer operators ($\mathbf{F}$ and $\mathbf{G}$), we may fill the ghost cell
positions using the same operator applied in the corresponding direction from adjacent panels
(as illustrated in Figure \ref{chp4-haloregion} a).
With this formulation of the flux computation, 
we can ensure that the PL07 splitting scheme (Table \ref{chp3-tab1}) eliminates the splitting error when $q$ is constant.
This flux formulation is referred to as \textbf{ET-PL07}.
...
\begin{table}[!h]
	...
	\end{tabular}
	\caption{Initial conditions considered in the numerical experiments.}
	\label{chp5-tab1} 
\end{table}
...
\begin{figure}[!htb]
	...
	\begin{subfigure}{0.45\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{gnomonic_equiangular_cs_256_adv_Q_ic2_vf2_SP-AVLT_PPM-L04_RK2_ET-ZA22-PR_interp3_t0_mercator}
		\caption{IC1. \label{chp5-ic1}}
	\end{subfigure}
	\begin{subfigure}{0.45\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{gnomonic_equiangular_cs_256_adv_Q_ic3_vf4_SP-AVLT_PPM-L04_RK2_ET-ZA22-PR_interp3_t0_mercator}
		\caption{IC2. \label{chp5-ic2}}
	\end{subfigure}

	\begin{subfigure}{0.45\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{gnomonic_equiangular_cs_256_adv_Q_ic4_vf2_SP-AVLT_PPM-L04_RK2_ET-ZA22-PR_interp3_t0_mercator}
