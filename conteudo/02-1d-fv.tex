...
of $\Omega \times[0,T]$, we introduce the grid functions $q^n \in \mathbb{R}^{N+2\nu}_P$ and $u^n \in \mathbb{R}^{N+1+2\nu}+P$. 
...
\end{align*}
We use the notation $\mathbb{R}^{N+1+2\nu}_{P}$ and $\mathbb{R}^{N+2\nu}_P$ to
...
Given $Q \in \mathbb{R}^{N+2\nu}_P$, we define the $p$-norm as
\begin{equation}
	\label{chp2-sec-not1}
	\|Q\|_{p,\Delta x}=
	\begin{cases}
...
\begin{prob}[1D FV-SL scheme]
	...
	\begin{equation}
		...
		\quad \forall n = 0, \ldots, N_T-1,
	\end{equation}
	where ${Q}^{n} \in \mathbb{R}^{N+2\nu}_P$ is intended to be an approximation
	of ${Q}(t^{n})\in \mathbb{R}^{N+2\nu}_P$ in some sense. We define
...
In particular, if the CFL number is less than 1, then $k=0$ and we need the ghost values $u_{-1+\frac{1}{2}}^n$ and $u_{N+\frac{3}{2}}^n$.
...
In this context, it is convenient to define the $\Delta x$-grid function $Q\in \mathbb{R}^{N+2\nu}_P$ with the entries given by $Q_i$.
...
Assuming that $Q^{n} \in \mathbb{R}^{N+2\nu}_P$ is known, our objective is to compute the values $Q^{n+1}$.
