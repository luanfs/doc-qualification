...
\end{figure}
To investigate the error convergence, we employ $(\Delta x^{(k)}, \Delta t^{(k)}, \lambda)$-discretizations with $\Delta x^{(k)} = 1/2^k$ for 
$k = 4, \ldots, 10$, $\Delta t^{(0)}=0.25$, $\lambda = 4$.
