...
In this chapter, we use the notation $\Omega=[a,b]$.
\begin{definition}[$\Delta x$-grid]\label{chp2-def-dxgrid}
	...
	we say that $\Omega_{\Delta x}= \{X_i \}_{i=1}^{N}$ is a $\Delta x$-grid for $\Omega$ if
	\begin{align*}
...
\begin{definition}[$\Delta x$-grid function]
	For a $\Delta x$-grid, we say that $Q$ is a $\Delta x$-grid function if $Q = (Q_1, \ldots, Q_N) \in \mathbb{R}^N$.
\end{definition}
...
Furthermore, considering functions $q, u: \Omega \times[0,T] \to \mathbb{R}$ and a $(\Delta x,\Delta t, \lambda)$-discretization
of $\Omega \times[0,T]$, we introduce the grid functions $q^n \in \mathbb{R}^{N}$ and $u^n \in \mathbb{R}^{N+1}$. 
