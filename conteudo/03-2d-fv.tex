...
\begin{definition}[$(\Delta x,\Delta y)$-grid]
	...
	we say that a $(N,M)$-tuple $\mathcal{D}=(\Omega_{ij})_{i=1,\ldots,N,j=1,\ldots,M}$ is a $(\Delta x, \Delta y)$-grid for $[a,b]\times [c,d]$ if
...
\begin{remark}
	...
	$q^n$ and $Q(t^n)$ , where ${q}^n_{ij} = {q}(x_i, y_j, t^{n})$, $Q(t^n)_{ij} = Q_{ij}(t^n)$, for $n=0, \ldots, N_T$.
...
\ldots, 10$. We introduce the relative error in the maximum norm:
\begin{equation*}
...
\begin{figure}[!htb]
	...
	a CFL number equal to $0.8$, $N=M=128$, and the initial condition is given by Equation \eqref{chp3-ic3}.
...
\begin{figure}[!htb]
	...
	applied to the linear advection problem using a velocity from Equation \eqref{chp3-vf2},
...
and monotonicity  being respected in the 2D case whenever we employ the monotonic limiters in the 1D subproblems.
