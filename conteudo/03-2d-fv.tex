...
\end{align*}
We use the notation $\mathbb{R}^{N \times M}_{\nu,P}$ represent the spaces of periodic $(\Delta x, \Delta y)$-grid functions.
...
\begin{prob}[2D-FV scheme]
    ...
    {G}_{i,j+\frac{1}{2}}^{n} 
    - {G}_{i,j-\frac{1}{2}}^{n}$ 
    and ${Q}^{n}\in \mathbb{R}^{N\times M}_{\nu,P}$ is intended to be an approximation
	of ${Q}(t_{n})\in \mathbb{R}^{N\times M}_{\nu,P}$ in some sense. We define ${Q}_{ij}^{0} = {Q}_{ij}(0)$ or
	${Q}_{ij}^{0} = {q}^0_{i,j}$.
...
\begin{definition}[Discrete divergence]
    ...
    $(\Delta x, \Delta y)$-grid function $\mathbb{D}^n(Q^n,u^n,v^n) \in \mathbb{R}^{N\times M}_{\nu,P}$
	given by:
	\begin{equation}
...
\end{align}
where $\tau^n \in \mathbb{R}^{N\times M}_{\nu,P}$ is the local truncation error (LTE).
...
\end{equation*}
where we are defining the grid function $\delta^n \in \mathbb{R}^{N\times M}_{\nu,P}$ of the exact divergence at the cell centers, namely
\begin{equation}
...
${G}^n_{i,j+\frac{1}{2}}(Q^n_{i,\times} ,\tilde{v}^n_{i,j+\frac{1}{2}})$ are compuute using the PPM flux
assuming that the CFL number is less than one (see Equation \eqref{chp-sec-flux:numerical-flux3}).
We introduce the auxiliary grid functions $\mathbf{F}, \mathbf{G} \in \mathbb{R}^{N\times M}_{nu}$ given by:
\begin{align*}
